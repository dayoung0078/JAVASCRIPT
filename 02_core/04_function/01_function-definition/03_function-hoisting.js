...
// 함수 선언문
function hello(name) {
    return `${name}님 안녕하세요!`; //함수를 정의한다.
}

// 함수 표현식
var hi = function(name) {
    return `${name} 안녕~`;
} // var hi - 변수 선언문 // 나머지 할당문
...
var hi; // hi 변수 선언문 (선언은 되어있지만 undifined가 뜬다.)

console.log(hello);
console.log(hi); // (선언은 되어있지만 undifined가 뜬다.)  
hi = function(name) {
    return `${name} 안녕~`;
}
...
//자바스크립트가 알아서 호이스팅 해준다.. 자바보다 낫네 ㅠ
...
호이스팅 : (함수 선언 보다 함수 호출이 윗 줄에 위치해도 실행되는 기능) 명사 뜻 : 끌어 올리기
